...
def test_no_negative_prices():
    ...
    """
    S, K, T, r, sigma = 100, 100, 1, 0.05, 0.5
...
def test_delta_relationship():
    ...
    """
    S, K, T, r, sigma = 100, 100, 1, 0.05, 0.2
    delta_c, _, _, _, _ = calculate_greeks(S, K, T, r, sigma, "call")
    delta_p, _, _, _, _ = calculate_greeks(S, K, T, r, sigma, "put")
    
    assert abs(delta_c - delta_p - 1.0) < 0.0001
...
def test_implied_volatility_round_trip():
    ...
    market_price = black_scholes(S, K, T, r, sigma_input, "call")
    
    # Ask the model to find the implied volatility
    sigma_found = calculate_implied_volatility(market_price, S, K, T, r, "call")
...
def test_monte_carlo_convergence():
    ...
    # Estimated Price 
    np.random.seed(42) 
    mc_price, _ = monte_carlo_simulation(S, K, T, r, sigma, N=10000, option_type="call")
